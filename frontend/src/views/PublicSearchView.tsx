...
export const PublicSearchView: React.FC<PublicSearchViewProps> = ({ onNavigateToLogin }) => {
  ...
  const therapeuticCategories = [
    { label: 'مسكنات', term: 'Panadol', icon: '💊' },
    { label: 'مضادات', term: 'Augmentin', icon: '🧪' },
    { label: 'الضغط', term: 'Concor', icon: '❤️' },
    { label: 'السكري', term: 'Glucophage', icon: '🩸' },
    { label: 'المعدة', term: 'Omeprazole', icon: '🩺' },
    { label: 'فيتامينات', term: 'Vitamin D3', icon: '🍊' },
    { label: 'حساسية', term: 'Zyrtec', icon: '🫁' },
    { label: 'أطفال', term: 'Brufen Syrup', icon: '👶' },
  ];
...
const IRAQ_COORDINATES: Record<string, { lat: number; lng: number }> = {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 font-sans flex flex-col selection:bg-emerald-500 selection:text-slate-950 w-full max-w-full overflow-x-hidden">
      ...
      <div className="bg-gradient-to-b from-slate-900 via-slate-900 to-slate-950 border-b border-slate-800/80 px-3 sm:px-8 py-6 sm:py-10 lg:py-12 w-full max-w-full overflow-x-hidden">
        <div className="max-w-4xl mx-auto space-y-4 sm:space-y-5">
          {/* Header Title */}
          <div className="text-center space-y-2">
            <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 bg-emerald-500/10 border border-emerald-500/20 rounded-full text-emerald-400 text-[10px] sm:text-xs font-black">
            ...
            {/* Live Metrics Row */}
            <div className="flex items-center justify-center gap-2 sm:gap-4 pt-1 flex-wrap text-xs text-slate-300 font-bold">
              <div className="flex items-center gap-1.5 bg-slate-800/60 px-3 py-1 rounded-full border border-slate-800">
                <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse"></span>
                <span>+{stats.totalMedicines.toLocaleString()} دواء</span>
              </div>
              <div className="flex items-center gap-1.5 bg-slate-800/60 px-3 py-1 rounded-full border border-slate-800">
                <Moon className="w-3 h-3 text-amber-400" />
                <span>خافرة 24/7</span>
              </div>
              <div className="flex items-center gap-1.5 bg-slate-800/60 px-3 py-1 rounded-full border border-slate-800">
          ...
          <div className="relative shadow-2xl rounded-3xl mt-3">
            ...
            <input
              ...
              className="w-full pr-12 pl-24 py-3.5 sm:py-4 bg-slate-900/90 border border-slate-700 rounded-3xl text-sm font-bold text-white placeholder:text-slate-500 focus:outline-hidden focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 shadow-inner"
            />
          ...
          {isListening && (
            <div className="p-2.5 bg-rose-500/20 border border-rose-500/40 rounded-2xl text-xs font-black text-rose-300 flex items-center justify-center gap-2 animate-pulse">
              <span className="w-2.5 h-2.5 rounded-full bg-rose-500"></span>
              <span>تكلّم الآن... 🎙️</span>
            </div>
          )}

          {/* GPS Error Alert */}
          {gpsError && (
            <div className="p-2.5 bg-amber-500/10 border border-amber-500/30 rounded-2xl text-xs font-bold text-amber-300 flex items-center gap-2">
          ...
          <div className="flex items-center gap-2 overflow-x-auto no-scrollbar pb-1 pt-1">
            ...
            >
              ...
              <span>{userCoords ? 'موقعي 📍' : 'الأقرب إليّ 📍'}</span>
            </button>
          ...
          {governorate && uniqueDistricts.length > 0 && (
            <div className="flex items-center gap-2 overflow-x-auto no-scrollbar pt-1">
              <span className="text-xs font-bold text-slate-400 whitespace-nowrap">المنطقة:</span>
              <button
                onClick={() => setDistrict('')}
      ...
      <div className="max-w-5xl mx-auto w-full px-4 sm:px-8 py-6 sm:py-8 flex-1 space-y-6 sm:space-y-8">
        {loading ? (
          ...
          <div className="text-center py-14 px-4 rounded-3xl bg-slate-900/60 border border-slate-800 space-y-3">
            <div className="w-14 h-14 rounded-full bg-slate-800/80 flex items-center justify-center mx-auto text-slate-600">
              <Pill className="w-7 h-7 stroke-[1.5]" />
            </div>
            <h4 className="text-base font-bold text-slate-300">
              {searchTerm ? 'غير متوفر' : 'ابحث عن دواء'}
            </h4>
            <p className="text-xs text-slate-500 max-w-md mx-auto">
              {searchTerm ? 'جرّب اسماً آخر أو صيدلية أخرى' : 'اكتب اسم الدواء للبحث'}
            </p>
          </div>
        ) : (
          <>
            ...
            {results.length > 0 && (
              <div className="space-y-3 sm:space-y-4">
                <div className="flex items-center justify-between text-xs font-bold text-slate-400 px-1">
                  <span>الصيدليات المتوفرة ({results.length})</span>
                  <span>{userCoords ? 'الأقرب 📍' : 'الأفضل سعراً'}</span>
                </div>
            ...
            {alternatives.length > 0 && (
              <div className="space-y-3 sm:space-y-4 pt-6 border-t border-slate-800">
                <div className="flex items-center justify-between px-1">
                  <div className="flex items-center gap-2">
                    <Layers className="w-5 h-5 text-indigo-400" />
                    <div>
                      <h3 className="text-sm font-black text-indigo-300">
                        بدائل متطابقة ({alternatives.length})
                      </h3>
                      <p className="text-[11px] text-slate-400">نفس الفعالية والتركيز</p>
                    </div>
                  </div>
                </div>
        ...
        <div className="p-5 sm:p-6 bg-gradient-to-r from-indigo-950 via-slate-900 to-indigo-950 rounded-3xl border border-indigo-800/60 shadow-xl flex flex-col md:flex-row items-center justify-between gap-4 sm:gap-6">
          <div className="space-y-1 text-center md:text-right">
            ...
            <h3 className="text-base sm:text-lg font-black text-white">انضم إلينا 🚀</h3>
            <p className="text-xs text-slate-400 max-w-lg font-medium">
              لأصحاب الصيدليات والمذاخر
            </p>
          </div>

          <button
            onClick={() => setShowJoinModal(true)}
            className="px-5 py-2.5 bg-gradient-to-r from-indigo-600 to-indigo-500 hover:from-indigo-500 hover:to-indigo-400 text-white rounded-2xl text-xs font-black shadow-lg shadow-indigo-600/30 transition-all active:scale-95 cursor-pointer whitespace-nowrap"
          >
            انضم الآن ⚡
          </button>
        </div>
        ...
        <div className="p-3.5 sm:p-4 bg-slate-900/60 rounded-2xl border border-slate-800/80 text-xs text-slate-400 flex items-start gap-3">
          <ShieldCheck className="w-4 h-4 sm:w-5 sm:h-5 text-emerald-500 shrink-0 mt-0.5" />
          <div className="space-y-0.5">
            <div className="font-bold text-slate-300 text-xs">إرشاد طبي 🩺:</div>
            <p className="text-[11px] text-slate-500 leading-relaxed">
              استشر الصيدلاني دائماً قبل الاستخدام.
            </p>
          </div>
        </div>
      </div>
      ...
      {showJoinModal && (
        <div className="fixed inset-0 bg-slate-950/70 backdrop-blur-xs flex items-center justify-center p-4 z-50 animate-in fade-in duration-200">
          <div className="bg-white text-slate-900 rounded-3xl p-5 sm:p-6 max-w-md w-full shadow-2xl border border-slate-200 space-y-4">
            <div className="flex items-center justify-between pb-3 border-b border-slate-100">
              <div className="flex items-center gap-2">
                <Building2 className="w-5 h-5 text-indigo-600" />
                <h3 className="font-black text-base">انضمام صيدلية</h3>
              </div>
              <button
            ...
            <form onSubmit={handleJoinPharmacySubmit} className="space-y-3">
              ...
              <div>
                <label className="block text-xs font-bold text-slate-700 mb-1">اسم الصيدلاني *</label>
                <input
                  type="text"
                  required
              ...
              <div className="pt-2 flex gap-2">
                <button
                  type="submit"
                  className="flex-1 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-xs font-black shadow-md shadow-emerald-600/30 flex items-center justify-center gap-1.5 cursor-pointer active:scale-95"
                >
      ...
      {/* 7. Footer */}
      <footer className="border-t border-slate-900 bg-slate-950 px-4 sm:px-6 py-5 text-center text-xs text-slate-500 font-medium">
        <div className="max-w-5xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-2">
          <div className="flex items-center gap-2 font-black text-slate-400">
            <ShieldCheck className="w-4 h-4 text-emerald-500" />
            <span>منصة دوائي 🇮🇶</span>
          </div>
          <div>حقوق النشر © {new Date().getFullYear()}</div>
        </div>
      </footer>
    </div>
  );
